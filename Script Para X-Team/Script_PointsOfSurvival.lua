...
PointsForSurvival_Read = function()
    ...
    Main.Configs["Schedule"] = {    -- wDay = Dia da semana [1~7], Hour = Horário do dia, AlertTime = tempo de anúncio/entrada;
	    [1] = { wDay = nil, Hour = "12:35:00", AlertTime = 3 },	
        [2] = { wDay = nil, Hour = "19:30:00", AlertTime = 3 },	
        [3] = { wDay = nil, Hour = "21:35:00", AlertTime = 3 },		
        [4] = { wDay = nil, Hour = "22:30:00", AlertTime = 3 }   
    };
    ...
    Main.Configs["MaxPlayers"]      = 20;       -- Máximo de participantes;
    ...
    Main.Configs["Reward1"]         = 3;        -- Valor para o 1º do ranking;
    Main.Configs["Reward2"]         = 2;        -- Valor para o 2º do ranking;
    Main.Configs["Reward3"]         = 1;        -- Valor para o 3º do ranking;
    Main.Configs["SpecialValue1"]   = 21;        -- Special Value da EventItemBag para o 1º do ranking; Itens inseridos no inventário.
    ...
    if Main.Stage == nil then
        ...
        for index in pairs(Main.Configs["Schedule"]) do
            ...
            if (Selected["wDay"] == nil or Selected["wDay"] == wDay) and (Selected["Hour"] == Hour) then
                ...
                LogPrint("[PANCADARIA]: Evento iniciado. Período de entrada liberado.");
                break;
            end;
        ...
        if (Main.Counter%60 == 0) then
            ...
            if Main.AlertTime == 0 then
                ...
                if (Main.UserCount >= Main.Configs["MinPlayers"]) then
                    ...
                    NoticeSendToAll(0,". : [ Evento Pancadaria ] : .");
                    ...
                    LogPrint("[Evento Pancadaria]: Evento iniciado. Tempo de entrada esgotado.");
                ...
                NoticeSendToAll(0, ". : [ Evento Pancadaria ] : .");
                NoticeSendToAll(0, string.format("O EVENTO INICIARÁ EM %d MINUTOS(s).", Main.AlertTime));
    ...
    elseif(Main.Stage == 2) then
        ...
        if Main.Stage ~= nil then
            ...
            for aIndex in pairs(Main.Players) do
                ...
                if EffectCheck(aIndex, 1) == 1 then -- ELFA
                    if GetObjectClass(aIndex) ~= 2 then EffectDel(aIndex, 1); UserInfoSend(aIndex); end;
                end;
                if EffectCheck(aIndex, 2) == 1 then -- ELFA 
                    if GetObjectClass(aIndex) ~= 2 then EffectDel(aIndex, 2); UserInfoSend(aIndex); end;
                end;
                if EffectCheck(aIndex, 4) == 1 then -- SM
                    if GetObjectClass(aIndex) ~= 0 then EffectDel(aIndex, 4); UserInfoSend(aIndex); end;
                end;
                if EffectCheck(aIndex, 5) == 1 then -- DL
                    if GetObjectClass(aIndex) ~= 4 then EffectDel(aIndex, 5); UserInfoSend(aIndex); end;
                end;
                if EffectCheck(aIndex, 8) == 1 then -- BK 
                    if GetObjectClass(aIndex) ~= 1 then EffectDel(aIndex, 8); UserInfoSend(aIndex); end;
                end;
